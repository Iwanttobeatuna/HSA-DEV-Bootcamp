...
const usersByPet = pet => {
  let newArray = database.users.filter(function(foo) {
    return foo.favPet == pet
  })
  return newArray
}
...
const collegeLookup = user => {
  let arr1 = database.users
  let arr2 = database.college
  let arr3 = arr2.map ((item,i) => Object.assign({}, item, arr1[i]))
  let arr4 = arr3.filter(function(fun) {
    return fun.firstName = user
  }) 
  return arr4.name + arr4.color
}
